...
namespace RVP
{
	...
	public class VehicleDamage : MonoBehaviour
	{
		...
		readonly ContactPoint nullContact = new();
		...
		void Start()
		{
			...
			for (int i = 0; i < deformMeshes.Length; i++)
			{
				...
				{
					verts = deformMeshes[i].mesh.vertices,
					initialVerts = deformMeshes[i].mesh.vertices
				};
			...
			{
				tempCols[i] = Instantiate(deformColliders[i].sharedMesh);
				colVertices[i] = new meshVerts
				{
					verts = deformColliders[i].sharedMesh.vertices,
					initialVerts = deformColliders[i].sharedMesh.vertices
				};
				damagedCols[i] = false;
			}
			...
			for (int i = 0; i < displaceParts.Length; i++) initialPartPositions[i] = displaceParts[i].localPosition;
		}
		...
		void OnCollisionEnter(Collision col)
		{
			if (hitTime == 0 && col.relativeVelocity.sqrMagnitude * damageFactor > 1 && strength < 1)
			{
				...
				{
					if (tr.InverseTransformPoint(curCol.point).y > collisionIgnoreHeight && GlobalControl.damageMaskStatic ==
					    (GlobalControl.damageMaskStatic | (1 << curCol.otherCollider.gameObject.layer)))
					{
						...
						{
							vp.CrashSnd.PlayOneShot(vp.CrashClips[Random.Range(0, vp.CrashClips.Length)],
					...
					if (colsChecked >= maxCollisionPoints) break;
				}
		...
		void DamageApplication(Vector3 damagePoint, Vector3 damageForce, float damageForceLimit, Vector3 surfaceNormal, ContactPoint colPoint,
			bool useContactPoint)
		{
			...
			if (useContactPoint)
			{
				...
				if (colPoint.otherCollider.attachedRigidbody) massFactor = Mathf.Clamp01(colPoint.otherCollider.attachedRigidbody.mass / rb.mass);
			}

			surfaceDot = Mathf.Clamp01(Vector3.Dot(surfaceNormal, normalizedVel)) *
			             (Vector3.Dot((tr.position - damagePoint).normalized, normalizedVel) + 1) * 0.5f;

			// Damage damageable parts
			for (int i = 0; i < damageParts.Length; i++)
			{
				curDamagePart = damageParts[i];
				damagePartFactor = colMag * surfaceDot * massFactor * Mathf.Min(clampedColMag * 0.01f,
					clampedColMag * 0.001f / Mathf.Pow(Vector3.Distance(curDamagePart.position, damagePoint), clampedColMag));

				// Damage motors
				Motor damagedMotor = curDamagePart.GetComponent<Motor>();
				if (damagedMotor) damagedMotor.health -= damagePartFactor * (1 - damagedMotor.strength);

				// Damage transmissions
				Transmission damagedTransmission = curDamagePart.GetComponent<Transmission>();
				if (damagedTransmission) damagedTransmission.health -= damagePartFactor * (1 - damagedTransmission.strength);
			}
			...
			for (int i = 0; i < deformMeshes.Length; i++)
			{
				...
				if (shattered)
				{
					...
					if (Vector3.Distance(curDamageMesh.transform.position, damagePoint) < colMag * surfaceDot * 0.1f * massFactor &&
					    colMag * surfaceDot * massFactor > shattered.breakForce) shattered.Shatter();
				}
					...
					for (int j = 0; j < meshVertices[i].verts.Length; j++)
					{
						...
						if (distClamp > 0.001f)
						{
							...
							if (seamKeeper == null || seamlessDeform)
							{
								...
								meshVertices[i].verts[j] +=
									(clampedTranslation - vertProjection * (usePerlinNoise
										? 1 + Mathf.PerlinNoise(meshVertices[i].verts[j].x * 100, meshVertices[i].verts[j].y * 100)
										: 1)) * surfaceDot * Mathf.Min(clampedColMag * 0.01f, distClamp) * massFactor;
							}
							else
							{
								seamLocalPoint = seamKeeper.InverseTransformPoint(curDamageMesh.transform.TransformPoint(meshVertices[i].verts[j]));
								meshVertices[i].verts[j] +=
									(clampedTranslation - Vector3.Project(normalizedVel, seamLocalPoint) *
										(usePerlinNoise ? 1 + Mathf.PerlinNoise(seamLocalPoint.x * 100, seamLocalPoint.y * 100) : 1)) * surfaceDot *
				...
				if (translation.sqrMagnitude > 0 && strength < 1)
				{
					...
					if (distClamp > 0.001f)
					{
						...
						else if (curDisplacePart.parent.GetComponent<DetachablePart>())
						{
							...
							{
								detachedPart.displacedAnchor += curDisplacePart.parent.InverseTransformDirection(clampedTranslation * surfaceDot *
									Mathf.Min(clampedColMag * 0.01f, distClamp) * massFactor);
							}
						}
		...
		void FinalizeDamage()
		{
			...
			for (int i = 0; i < deformMeshes.Length; i++)
			{
				if (damagedMeshes[i])
				{
					...
					if (calculateNormals) tempMeshes[i].RecalculateNormals();
		...
		public void Repair()
		{
			...
			for (int i = 0; i < damageParts.Length; i++)
			{
				if (damageParts[i].TryGetComponent(out Motor motor))
					motor.health = 1;

				if (damageParts[i].TryGetComponent(out Transmission transmission))
					transmission.health = 1;
			}

			// Restore deformed meshes
			for (int i = 0; i < deformMeshes.Length; i++)
			{
				for (int j = 0; j < meshVertices[i].verts.Length; j++) meshVertices[i].verts[j] = meshVertices[i].initialVerts[j];
			...
			for (int i = 0; i < deformColliders.Length; i++)
			{
				for (int j = 0; j < colVertices[i].verts.Length; j++) colVertices[i].verts[j] = colVertices[i].initialVerts[j];
			...
			for (int i = 0; i < displaceParts.Length; i++)
			{
				...
				else if (curDisplacePart.parent.GetComponent<DetachablePart>()) curDisplacePart.parent.GetComponent<DetachablePart>().Reattach();
			...
			// Fix hover wheels
			foreach (HoverWheel curHoverWheel in vp.HoverWheels) curHoverWheel.Reattach();
		}
		...
		void OnDestroy()
		{
			foreach (Transform curPart in displaceParts)
				if (curPart)
				{
					...
					else if (curPart.parent.GetComponent<DetachablePart>() && curPart.parent.parent == null) Destroy(curPart.parent.gameObject);
				}
		}
	}
